...
for time in range(T):
    ...
    for r1 in [2,0,1,3]:
        while 1:
            ...
            if [x1,y1] == cleaner[0]:
                break
            if new_graph[x1][y1]!=0 and new_graph[x1-dx[r1]][y1-dy[r1]] == 0:
                temp = new_graph[x1][y1]
                new_graph[x1][y1] = 0
                new_graph[x1-dx[r1]][y1-dy[r1]] = temp
            if new_graph[x1][y1]!=0 and new_graph[x1-dx[r1]][y1-dy[r1]] == -1:
                new_graph[x1][y1] = 0
    # 아래 공기 청정기
    x2,y2 = cleaner[1]
    for r2 in [1,0,2,3]:
        while 1:
            ...
            if [x2,y2] == cleaner[1]:
                break
            if new_graph[x2][y2] != 0 and new_graph[x2-dx[r2]][y2-dy[r2]] == 0:
                temp = new_graph[x2][y2]
                new_graph[x2][y2] = 0
                new_graph[x2-dx[r2]][y2-dy[r2]] = temp
            if new_graph[x2][y2] != 0 and new_graph[x2-dx[r2]][y2-dy[r2]] == -1:
                new_graph[x2][y2] = 0
